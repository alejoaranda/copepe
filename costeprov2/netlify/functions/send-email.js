...
// Versión completa con lógica de enlace temporal de 12 horas.
...
async function appendToSheet(email, token) {
  try {
    ...
    const sheet = doc.sheetsByIndex[0]; // Selecciona la primera hoja.

    // Guarda la fecha en formato ISO, que es el estándar universal para fechas y horas.
    const timestampISO = new Date().toISOString();
    
    // Añade la nueva fila con los datos. Asegúrate de que los encabezados en tu Sheet coincidan.
    await sheet.addRow({ 
      Email: email, 
      TokenUnico: token, 
    ...
    console.log(`Email ${email} y su token han sido añadidos a Google Sheets.`);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (data.formType === 'trial') {
      ...
      // 2. Guardar la información en Google Sheets.
      await appendToSheet(data.email, token);

      // 3. Construir el enlace de descarga especial que apunta a nuestra otra función.
      // ¡IMPORTANTE! Reemplaza "TU_DOMINIO.com" por tu dominio real.
      const downloadLink = `https://TU_DOMINIO.com/.netlify/functions/descargar-prueba?token=${token}`;

      // 4. Enviar el email al cliente con la plantilla elegante y el enlace temporal.
      ...
      // 5. Notificación para ti (opcional pero recomendado).
      const notificationToOwner = {
        ...
        html: `<p>El usuario <strong>${data.email}</strong> ha recibido su enlace de descarga temporal.</p>`
      };
